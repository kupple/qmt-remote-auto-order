...
class qmt_trader:
 ...
 def sell(self,security='600031.SH',
  ...
  price_type = get_qmt_price_type(security,order_style_str,'sell')
  ...
  if order_volume>0:
   ...
   G.logger.info("交易类型{} 代码{} 价格{} 数量{} 订单编号{}".format(order_type,stock_code,price,order_volume,fix_result_order_id),extra={
      "showMessage": True
    })
  #  fix_result_order_id - 1有问题
   return fix_result_order_id
  else:
   G.logger.error("卖出 标的{} 价格{} 委托数量{}小于0有问题".format(stock_code,price,order_volume),extra={
      "showMessage": True
    })   
 ...
 def buy(self,security='600031.SH',
  ...
  price_type = get_qmt_price_type(security, order_style_str,'buy')
  # 对交易回调进行订阅，订阅后可以收到交易主推，返回0表示订阅成功
  subscribe_result = self.xt_trader.subscribe(self.acc)
  stock_code =self.adjust_stock(stock=security)
  ...
  if order_volume>0:
   ...
   G.logger.error("买入 标的{} 价格{} 委托数量{}小于0有问题".format(stock_code,price,order_volume),extra={
      "showMessage": True
    })
