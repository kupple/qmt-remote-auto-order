...
class qmt_trader:
    ...
    def adjust_stock(self, stock='600031.SH'):
        ...
        调整代码
        """
        if stock[-2:] in ['SH', 'SZ', 'sh', 'sz']:
            stock = stock.upper()
        else:
            if (stock[:3] in ['600', '601', '603', '688', '510', '511',
                             '512', '513', '515', '113', '110', '118', '501'] or
                    stock[:2] in ['11']):
                stock = stock + '.SH'
            else:
                stock = stock + '.SZ'
    ...
    def sell(self, security='600031.SH',
        ...
        if order_volume > 0:
            ...
            G.logger.info(
                "交易类型{} 代码{} 价格{} 数量{} 订单编号{}".format(
                    order_type, stock_code, price, order_volume, fix_result_order_id
                ),
            ...
            G.logger.error(
                "卖出 标的{} 价格{} 委托数量{}小于0有问题".format(stock_code, price, order_volume),
                extra={"showMessage": True}
            )
    ...
    def buy(self, security='600031.SH',
        ...
        if order_volume > 0:
            ...
            G.logger.error(
                "买入 标的{} 价格{} 委托数量{}小于0有问题".format(stock_code, price, order_volume),
                extra={"showMessage": True}
            )
    ...
    def connect(self, callback):
        ...
        account_type账户内类型
        """
        G.logger.info("正在连接QMT", extra={"showMessage": True})
    ...
    def reverse_repurchase_of_treasury_bonds(self, security='131810.SZ',
        ...
        if order_volume > 0:
            ...
            text = '国债逆回购交易类型{} 代码{} 价格{} 数量{} 订单编号{}'.format(
                order_type, stock_code, price, order_volume, fix_result_order_id
            )
            return True, text
        else:
            text = '国债逆回购卖出 标的{} 价格{} 委托数量{}小于0有问题'.format(
                stock_code, price, order_volume
            )
            return False, text
