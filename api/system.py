...
class System():
    ...
    def system_py2js(self, func, info):
        ...
        if func == 'pythonMsg':
            if info['type'] is '0':
                logging.info(str(info['msg']))
            elif info['type'] is '1':
                logging.info(str(info['msg']))
            elif info['type'] is '2':
                logging.error(str(info['msg']))
    ...
    def method_one(self):
        print(f"Running method_one on {threading.current_thread().name}")

    def method_two(self):
        print(f"Running method_two on {threading.current_thread().name}")
