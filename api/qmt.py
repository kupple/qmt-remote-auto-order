...
class MyXtQuantTraderCallback(XtQuantTraderCallback):
  ...
  def on_order_stock_async_response(self, response):
    ...
    if response.order_remark and response.strategy_name:
      ...
      self.sql.save_trade(response,{
        "orders_id": orderId
      })
      # self.sql.update_order(orderId,status = 1,fix_result_order_id = response.order_id)
...
class QMT:
  def __init__(self,sql):
    ...
    self.sql = sql
    self.callback = MyXtQuantTraderCallback(sql)
    
  def connectQMT(self):
    # 获取配置
    config = Database().get_setting_config()
    print(config)
    self.qmt_trader = qmt_trader(config['mini_qmt_path'], config['client_id'])
  ...
  def manage_qmt_trader(self,data):
    ...
    try:        
      ...
      # 创建空订单
      oderId = self.sql.save_order(saveData)
      oderId = str(oderId)
      ...
      task = next((item for item in Database.task_list if item.get('strategy_code') == strategy_id), None)
      return
      if task:
        if task['is_open'] == 1:
